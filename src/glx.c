...
void glx_displaylist_create(struct glx_displaylist* x, bool has_color, bool has_normal) {
	...
	x->client_vertex = NULL;
	x->client_color = NULL;
	x->client_normal = NULL;
	x->client_alloc_size = 0;
#endif
...
#ifdef __EMSCRIPTEN__
	/* Copy data into owned buffers so they survive tesselator_free */
	{
		size_t lv = ((type == GLX_DISPLAYLIST_NORMAL) ? sizeof(GLshort) : sizeof(GLfloat)) * 3;
		size_t lc = x->has_color ? (sizeof(GLubyte) * 4) : 0;
		size_t ln = x->has_normal ? (sizeof(GLbyte) * 3) : 0;
		size_t total = size * (lv + lc + ln);

		if(total > x->client_alloc_size) {
			x->client_alloc_size = total;
			x->client_vertex = realloc(x->client_vertex, total);
		}

		memcpy(x->client_vertex, vertex, size * lv);
		if(x->has_color && color)
			memcpy((char*)x->client_vertex + size * lv, color, size * lc);
		if(x->has_normal && normal)
			memcpy((char*)x->client_vertex + size * (lv + lc), normal, size * ln);

		if(x->has_color)
			x->client_color = (char*)x->client_vertex + size * lv;
		if(x->has_normal)
			x->client_normal = (char*)x->client_vertex + size * (lv + lc);
	}
	return;
#endif
...
#ifdef __EMSCRIPTEN__
	/* Use client-side vertex arrays for Emscripten */
	if(x->size == 0) return;

	glEnableClientState(GL_VERTEX_ARRAY);

	switch(type) {
		case GLX_DISPLAYLIST_NORMAL: glVertexPointer(3, GL_SHORT, 0, x->client_vertex); break;
		case GLX_DISPLAYLIST_POINTS:
		case GLX_DISPLAYLIST_ENHANCED: glVertexPointer(3, GL_FLOAT, 0, x->client_vertex); break;
	}

	if(x->has_color) {
		glEnableClientState(GL_COLOR_ARRAY);
		glColorPointer(4, GL_UNSIGNED_BYTE, 0, x->client_color);
	}

	if(x->has_normal) {
		glEnableClientState(GL_NORMAL_ARRAY);
		glNormalPointer(GL_BYTE, 0, x->client_normal);
	}
...
#ifdef __EMSCRIPTEN__
	/* On Emscripten, GL_COLOR_MATERIAL + vertex arrays with lighting causes
	   all-white colors in legacy GL emulation. Use simple fog-only approach. */
	glEnable(GL_FOG);
...
void glx_disable_sphericalfog() {
	...
	glDisable(GL_FOG);
#ifdef __EMSCRIPTEN__
	/* Lighting was not enabled on Emscripten */
#else
	glDisable(GL_COLOR_MATERIAL);
	glDisable(GL_LIGHT1);
	glDisable(GL_LIGHTING);
