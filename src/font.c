...
static float* font_vertex_buffer;
static float* font_coords_buffer;
...
void font_init() {
	font_vertex_buffer = malloc(512 * 8 * sizeof(float));
...
static struct font_backed_data* font_find(float h) {
	...
	/* Convert single-channel alpha to RGBA for WebGL compatibility */
	{
		unsigned char* rgba = malloc(f.w * f.h * 4);
		CHECK_ALLOCATION_ERROR(rgba)
		for(int i = 0; i < f.w * f.h; i++) {
			rgba[i * 4 + 0] = 255; /* R */
			rgba[i * 4 + 1] = 255; /* G */
			rgba[i * 4 + 2] = 255; /* B */
			rgba[i * 4 + 3] = ((unsigned char*)temp_bitmap)[i]; /* A */
		}
...
void font_render(float x, float y, float h, char* text) {
	...
	glVertexPointer(2, GL_FLOAT, 0, font_vertex_buffer);
