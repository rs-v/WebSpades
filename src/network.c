...
#ifdef __EMSCRIPTEN__
	char* os = "BetterSpades (Web) " GIT_COMMIT_HASH;
...
#ifdef __EMSCRIPTEN__
		ws_send(network_send_tmp, len + 1);
#else
...
int network_connect_sub(char* ip, int port, int version) {
	...
	*network_custom_reason = 0;
	...
	char url[256];
	snprintf(url, sizeof(url), "ws://%s:%d/ws", ip, port);
	log_info("WebSocket connecting to %s", url);

	if(!ws_connect(url)) {
		chat_showpopup("WebSocket connection failed", 3.0F, rgb(255, 0, 0));
		return 0;
	}

	/* Wait for WebSocket to connect (up to 3 seconds, polling) */
	float start = window_time();
	while(window_time() - start < 3.0F) {
		if(ws_is_connected()) {
		...
		if(!ws_is_connecting()) {
			chat_showpopup("Connection failed", 3.0F, rgb(255, 0, 0));
			return 0;
		}
...
int network_connect(char* ip, int port) {
	log_info("Connecting to %s at port %i (WebSocket)", ip, port);
	if(network_connected) {
		network_disconnect();
	}
	/* WebSocket connects directly, no version negotiation needed */
	return network_connect_sub(ip, port, 0);
}
#else
int network_connect_sub(char* ip, int port, int version) {
	ENetAddress address;
...
int network_connect_string(char* addr) {
	...
	int port;
#ifdef __EMSCRIPTEN__
	/* For WebSocket, accept plain IP:port without aos:// prefix */
	if(strncmp(addr, "aos://", 6) != 0) {
		/* Treat as plain ip:port */
		char* port_start = strchr(addr, ':');
		if(port_start) {
			strncpy(ip, addr, port_start - addr);
			ip[port_start - addr] = 0;
			port = strtoul(port_start + 1, NULL, 10);
		} else {
			strcpy(ip, addr);
			port = 32887;
		}
		log_info("Direct connect to %s:%d", ip, port);
...
int network_update() {
	if(network_connected) {
		...
		if(!ws_is_connected()) {
			...
			chat_showpopup("Connection lost", 10.0F, rgb(255, 0, 0));
		...
		/* Process all received WebSocket messages (must fit largest packet: MapChunk = 8192+1 bytes) */
		uint8_t recv_buf[16384];
		int recv_len;
		while((recv_len = ws_recv(recv_buf, sizeof(recv_buf))) > 0) {
			network_stats[0].ingoing += recv_len;
			int id = recv_buf[0];
			if(*packets[id]) {
				log_debug("Packet id %i", id);
				(*packets[id])(recv_buf + 1, recv_len - 1);
			} else {
				log_error("Invalid packet id %i, length: %i", id, recv_len - 1);
			}
