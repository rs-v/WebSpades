...
void chunk_init() {
	...
	log_info("Chunk generation: single-threaded (Emscripten)");
#else
...
#ifdef __EMSCRIPTEN__
	/* Emscripten has no worker threads - build all chunks synchronously */
	for(int k = CHUNKS_PER_DIM / 2; k >= 0; k--) {
		for(int i = k; i < CHUNKS_PER_DIM - k; i++) {
			...
			for(size_t j = 0; j < sizeof(build) / sizeof(*build); j++) {
				...
				struct libvxl_chunk_copy blocks;
				map_copy_blocks(&blocks, c->x * CHUNK_SIZE, c->y * CHUNK_SIZE);

				if(settings.greedy_meshing)
					chunk_generate_greedy(&blocks, c->x * CHUNK_SIZE, c->y * CHUNK_SIZE, &result.tesselator,
										  &result.max_height);
				else
					chunk_generate_naive(&blocks, &result.tesselator, &result.max_height, settings.ambient_occlusion);

				size_t chunk_x = c->x * CHUNK_SIZE;
				size_t chunk_y = c->y * CHUNK_SIZE;
				uint32_t last_position = 0;
				for(int m = blocks.blocks_sorted_count - 1; m >= 0; m--) {
					struct libvxl_block* blk = blocks.blocks_sorted + m;
					if(blk->position != last_position || m == blocks.blocks_sorted_count - 1) {
						last_position = blk->position;
						int bx = key_getx(blk->position);
						int bz = key_gety(blk->position);
						uint32_t* out = result.minimap_data + (bx - chunk_x + (bz - chunk_y) * CHUNK_SIZE);
						if((bx % 64) > 0 && (bz % 64) > 0)
							*out = rgb2bgr(blk->color) | 0xFF000000;
						else
							*out = rgba(255, 255, 255, 255);
				...
				c->max_height = result.max_height;
				c->updated = false;
				tesselator_glx(&result.tesselator, &c->display_list);
				glBindTexture(GL_TEXTURE_2D, texture_minimap.texture_id);
				glTexSubImage2D(GL_TEXTURE_2D, 0, c->x * CHUNK_SIZE, c->y * CHUNK_SIZE,
								CHUNK_SIZE, CHUNK_SIZE, GL_RGBA, GL_UNSIGNED_BYTE, result.minimap_data);
				glBindTexture(GL_TEXTURE_2D, 0);
				tesselator_free(&result.tesselator);
				free(result.minimap_data);
...
#ifdef __EMSCRIPTEN__
static bool iterate_chunk_updates_sync(void* key, void* value, void* user) {
	struct chunk_work_packet* work = (struct chunk_work_packet*)value;
	...
	/* Upload result directly */
	if(!result.chunk->created) {
		glx_displaylist_create(&result.chunk->display_list, true, false);
		result.chunk->created = true;
	}
	result.chunk->max_height = result.max_height;
	result.chunk->updated = false;
	tesselator_glx(&result.tesselator, &result.chunk->display_list);
	glBindTexture(GL_TEXTURE_2D, texture_minimap.texture_id);
	glTexSubImage2D(GL_TEXTURE_2D, 0, result.chunk->x * CHUNK_SIZE, result.chunk->y * CHUNK_SIZE,
					CHUNK_SIZE, CHUNK_SIZE, GL_RGBA, GL_UNSIGNED_BYTE, result.minimap_data);
...
#ifdef __EMSCRIPTEN__
	ht_iterate(&chunk_block_queue, NULL, iterate_chunk_updates_sync);
