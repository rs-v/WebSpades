...
The native C client can be compiled to WebAssembly using [Emscripten](https://emscripten.org/). This runs BetterSpades directly in the browser with no plugins.

Install the Emscripten SDK (emsdk), then configure and build:
...
Serve the output with any HTTP server and open `client.html`:

```bash
cd BetterSpades/build-wasm/BetterSpades
...
- Single-threaded chunk building (no worker threads in WASM)
- Client-side vertex arrays instead of VBOs
- WebSocket networking via a custom JS library (`library_websocket.js`)
- Resources are preloaded into the virtual filesystem
